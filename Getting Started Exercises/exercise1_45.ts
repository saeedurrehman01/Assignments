...
var name3: string = "Albert Einstein";
var message1: string = `${name3} once said, “A person who never made a mistake never tried anything new.”`;
...
// True
let strng5: string = "MOUSE";
let strng6: string = "mouse";
...
let strng7: string = "MOUSE";
let strng8: string = "Mouse";
...
let items: string[] = ["eggs", "bottle", "toothbrush", "chips"];
...
var alien_color_3: string = "yellow";
...
var alien_color_4: string = "green";
...
var favorite_fruits: string[] = ["Mango", "Apple", "Strawberry"];
...
var current_users: string[] = ["Saeed", "Ali", "Zain", "Talha", "Saad"];

var new_users: string[] = ["Irzam", "Haroon", "Bilal", "Zain", "Raza"];

for (let name of new_users) {
  var usernameExists = current_users.some(
    (currentUsername) => currentUsername.toLowerCase() === name.toLowerCase()
  );
  if (usernameExists) {
    console.log(
      `The username '${name}' is not available. Please choose a different one.`
    );
  } else {
...
var ordianArray: number[] = [1, 2, 3, 4, 5, 6, 7, 8, 9];
...
var favPizza: string[] = ["Tikka", "Pepperoni", "Bihari Kebab", "Special"];
...
console.log(
  `I like Pizza. I like desi flavor Pizza as well as English flavor Pizza. I really Love Pizza`
);
...
var animalsArray: string[] = ["cat", "dog", "parrot"];
...
function make_shirt(
  ...
  console.log(
    `The size of the shirt is ${size}. The message to be printed on the shirt is as following "${message}"`
  );
}
...
type album1 = { artistName: string; albumTitle: string; tracks?: number };
...
var modifiedMagicians: string[] = make_great(magicians);
...
function make_sandwich(itemsOnSandwich: string[]): any {
